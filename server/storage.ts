...
export class DatabaseStorage implements IStorage {
  ...
  async getGasFeeStats(): Promise<{
    ...
    const gasPrices = recentBlocks
      .filter((b: { baseFeePerGas: string | null }) => b.baseFeePerGas)
      .map((b: { baseFeePerGas: string | null }) => BigInt(b.baseFeePerGas || 0));
    ...
    const sum = gasPrices.reduce((a: bigint, b: bigint) => a + b, BigInt(0));
    const avg = sum / BigInt(gasPrices.length);
    const min = gasPrices.reduce((a: bigint, b: bigint) => a < b ? a : b);
    const max = gasPrices.reduce((a: bigint, b: bigint) => a > b ? a : b);
